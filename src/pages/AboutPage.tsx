  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50/80 to-white py-12 md:py-16">
      ...
      <div className="mx-auto max-w-5xl px-4 md:px-6">
        ...
        <h1 className="mt-4 text-3xl font-black tracking-tight text-slate-900 md:text-4xl">{t("publicAbout.title")}</h1>
        ...
        {isError ? (
          <div className="mt-10 rounded-2xl border border-red-100 bg-red-50 p-6 text-sm text-red-800">
            {t("publicAbout.loadError")}
          </div>
        ) : null}

        {!isLoading && !isError && !description && !mission ? (
        ...
        {about ? (
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#EE7C11]/20 bg-[#EE7C11]/5 p-5 shadow-sm">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#EE7C11]/15 text-[#EE7C11]">
                  <Target className="h-5 w-5" />
                </div>
                <h3 className="mt-3 text-sm font-bold text-slate-900">{t("publicAbout.mission")}</h3>
                <p className="mt-2 text-xs leading-relaxed text-slate-600 md:text-sm">{mission || "—"}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-xs leading-relaxed text-slate-600 md:text-sm">{vision || "—"}</p>
              </div>
            </div>
          </div>
        ) : null}
        ...
        <div id="policies" className="scroll-mt-24 mt-10 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          ...
          <Link to="/terms" className="mt-3 inline-block text-sm font-semibold text-[#EE7C11] hover:underline">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-4 rounded-2xl border border-slate-200 bg-white px-6 py-8 shadow-sm">
          <Users className="h-8 w-8 text-[#EE7C11]" />
          <div className="text-center">
            <p className="text-sm font-semibold text-slate-900">{t("publicAbout.joinTitle")}</p>
            <div className="mt-3 flex flex-wrap justify-center gap-3">
              <Link
                to="/signup"
                className="rounded-xl bg-[#EE7C11] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#d9700e]"
              >
                {t("header.actions.signUp")}
              </Link>
              <Link
                to="/faq"
                className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-800 transition hover:border-[#EE7C11] hover:text-[#EE7C11]"
              >
