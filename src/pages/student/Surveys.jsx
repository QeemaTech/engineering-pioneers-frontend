    ...
    <motion.div
      ...
      className={`group relative overflow-hidden rounded-2xl border bg-white shadow-sm transition-shadow hover:shadow-md dark:bg-[#1E293B] ${
        session.alreadySubmitted
          ...
          : "border-orange-200/70 dark:border-orange-700/40"
      }`}
    >
      {/* Side accent bar */}
      <div
        className={`absolute start-0 top-0 h-full w-1 rounded-s-2xl ${
          session.alreadySubmitted ? "bg-emerald-500" : "bg-[#EE7C11]"
        }`}
      />
      ...
      <div className="flex items-center gap-4 p-5 ps-6">
        ...
        <div
          className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${
            session.alreadySubmitted
              ...
              : "bg-orange-100 text-[#EE7C11] dark:bg-orange-900/20"
          }`}
        >
          {session.alreadySubmitted ? (
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          {session.alreadySubmitted ? (
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-orange-100 px-2.5 py-1 text-xs font-semibold text-[#EE7C11] dark:bg-orange-900/20">
          ...
          {!session.alreadySubmitted && (
            <button
              ...
              className="flex items-center gap-1 rounded-xl bg-[#EE7C11] px-3 py-1.5 text-xs font-bold text-white shadow-sm shadow-orange-500/20 transition hover:bg-orange-600 active:scale-95"
            >
              {isRtl ? "تعبئة الاستبيان" : "Fill Survey"}
...
function EmptyState({ filter, isRtl }) {
  const messages = {
    ...
    pending: {
      ar: "لا توجد استبيانات في الانتظار — أحسنت! 🎉",
      en: "No pending surveys — great job! 🎉",
    },
    completed: {
      ar: "لم ترسل أي استبيان بعد.",
      en: "No surveys submitted yet.",
    },
  };
...
export default function StudentSurveys() {
  ...
  const [activeSession, setActiveSession] = useState(null); // session to fill

  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const { data } = await api.get(endpoints.student.surveySessions);
  ...
  // After modal submits, refresh the list so the card flips to ✅
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">
              {isRtl ? "نماذج الاستبيان" : "Survey Forms"}
            </h1>
            <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
              {isRtl
                ? "قيّم جلساتك المنتهية وساعدنا على التحسين"
                : "Rate your ended sessions and help us improve"}
            </p>
          </div>
        </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            {[
              ...
              },
              {
                ...
                value: pendingCount,
                color: "from-[#EE7C11] to-orange-500",
              },
              {
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="h-9 w-full rounded-xl border border-slate-200 bg-white ps-9 pe-3 text-sm text-slate-700 placeholder-slate-400 outline-none transition focus:border-[#EE7C11]/40 focus:ring-1 focus:ring-[#EE7C11]/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 sm:w-56"
          />
        </div>
      </div>
