...
export default function InstructorLiveSessions() {
  ...
  const loadData = async () => {
    try {
      ...
      const [sessRes, courseRes] = await Promise.all([
        client.get("/instructor/live-sessions"),
        client.get("/instructor/courses?limit=100"),
      ]);
      setSessions(sessRes.data?.data || []);
      setCourses(courseRes.data?.data?.courses || courseRes.data?.data || []);
    } catch (err) {
      toast.error(getErrorMessage(err, "Failed to load scheduled live sessions."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSession) {
        await client.patch(`/instructor/live-sessions/${editingSession.id}`, payload);
        toast.success(isRtl ? "تم تعديل الجلسة بنجاح" : "Live session updated successfully.");
      } else {
        await client.post("/instructor/live-sessions", payload);
        toast.success(isRtl ? "تمت جدولة الجلسة بنجاح" : "Live session scheduled successfully.");
      }
      setModalOpen(false);
      loadData();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await client.delete(`/instructor/live-sessions/${id}`);
      toast.success(isRtl ? "تم حذف الجلسة المباشرة بنجاح" : "Live session deleted successfully.");
      loadData();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={isRtl ? "إدارة بث الحصص المباشرة" : "Live Sessions"}
        subtitle={isRtl ? "جدولة وإدارة حصص البث المباشر المجانية والمدفوعة." : "Schedule and manage your standalone or course live sessions."}
        action={
          <button
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/8 dark:bg-[#1A1A22]">
        <div className="relative max-w-md">
          ...
          <input
            ...
            placeholder={isRtl ? "بحث باسم الحصة أو الكورس..." : "Search sessions or courses..."}
          />
        </div>
      </div>
                ...
                <div>
                  ...
                  {r.course && (
                    <span className="text-xs text-slate-400">
                      {isRtl ? "كورس: " : "Course: "} {r.course.title}
                    </span>
                  )}
                </div>
              ),
