...
const ShapeGrid = ({
  ...
  size,
  hoverFillColor: propHoverFillColor = '#222',
  hoverColor,
  shape = 'square',
  hoverTrailAmount = 0,
  className = ''
}) => {
  const squareSize = size !== undefined ? size : propSquareSize;
  const hoverFillColor = hoverColor !== undefined ? hoverColor : propHoverFillColor;
  ...
  useEffect(() => {
    ...
    const eventSource = canvas.closest('section') || canvas.parentElement || canvas;

    eventSource.addEventListener('mousemove', handleMouseMove);
    eventSource.addEventListener('mouseleave', handleMouseLeave);
    ...
    return () => {
      ...
      eventSource.removeEventListener('mouseleave', handleMouseLeave);
    };
